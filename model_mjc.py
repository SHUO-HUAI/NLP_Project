...
class Model(nn.Module):
    def __init__(self, dic, emb_dim=128, hidden_dim=256, max_enc=400, max_oovs=100):
        ...
        self.encoder = nn.LSTM(input_size=self.embed_size,
                               ...
                               bidirectional=True)
        self.decoder = nn.LSTM(input_size=self.embed_size,
                               hidden_size=self.hidden_size, batch_first=True)

        # linear weights
        self.W_init = nn.Linear(self.hidden_size * 2,
                                self.hidden_size)  # for using encoder's hidden state as decoder input

        self.wh = nn.Linear(self.hidden_size * 2, 1)  # for changing context vector into a scalar
        self.ws = nn.Linear(self.hidden_size, 1)  # for changing hidden state into a scalar
        self.wx = nn.Linear(self.embed_size, 1)  # for changing input embedding into a scalar

        self.Wh = nn.Linear(self.hidden_size * 2, self.hidden_size)  # for obtaining e from encoder hidden states
        self.Ws = nn.Linear(self.hidden_size, self.hidden_size)  # for obtaining e from current state
        self.Wc = nn.Linear(self.max_enc, self.hidden_size)  # for obtaining e from context vector
        self.v = nn.Linear(self.hidden_size, 1)  # for changing to scalar

        self.V1 = nn.Linear(self.hidden_size * 3, self.hidden_size * 3)
        self.V2 = nn.Linear(self.hidden_size * 3, self.vocab_size)
    ...
    def forward(self, input, target, train=True):
        ...
        unked_inputs = get_unked(input, self.dictionary)
        unked_target = get_unked(target, self.dictionary)

        # batch_size = input.size(0)

        # inputs = to_cuda(inputs)
        encoder_input = to_cuda(unked_inputs)
        decoder_input = to_cuda(unked_target)
        ...
        if train:
            ...
            ones = np.ones([b]) * self.word2idx['<SOS>']
            next_input = to_cuda(torch.Tensor((ones)))
        ...
        for i in range(tar_seq - 1):
            ...
            # else:
            attn1 = self.Wh(encoder_out.contiguous().view(-1, encoder_out.size(2))) + self.Ws(state.squeeze()).repeat(
            ...
            dup_attn_sum = to_cuda(dup_attn_sum)
            ...
            p_copy = to_cuda(Variable(p_copy))
